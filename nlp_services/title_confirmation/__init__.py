"""
This submodule is used for referencing strings against data sources that
connote entities
"""
...
def confirm(title, wiki_id=None, use_wikipedia=None):
    ...
    An entity is defined here as being germane to the scope of the universe of
    discourse
    The universe of discourse here is either wikipedia titles or titles in a
    given wiki
    ...
    if wiki_id is not None:
        can_confirm = preprocess(title) in AllTitlesService().get_value(
            wiki_id, [])
    if not can_confirm and use_wikipedia:
        can_confirm = check_wp(title)
    return can_confirm
...
def canonical(title, wiki_id):
    ...
    :rtype: str
    """
    redirects = RedirectsService().get_value(wiki_id)
